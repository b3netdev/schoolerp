...
const Classes = () => {
  const { getClasses, addclass, updateclass, deleteclass, restoreclass, hardDeleteclass } = useClass();
  ...
  const [restoreItem, setRestoreItem] = useState<ClassItem | null>(null);
  const [permanentDeleteItem, setPermanentDeleteItem] = useState<ClassItem | null>(null);
  ...
  const handleRestore = async () => {
    ...
    await restoreclass(restoreItem.id!);
    setRestoreItem(null);
  };
  ...
  const handlePermanentDelete = async () => {
    ...
    await hardDeleteclass(permanentDeleteItem.id!);
  ...
  const handleDeleteClick = (row: Record<string, unknown>) => {
    ...
    if (selectedClass) {
      if (statusFilter === "trash") {
        ...
        setPermanentDeleteItem(selectedClass);
      } else {
