...
const fields: FieldDef[] = [
  ...
  },
  {
    ...
    /**
     * The async availability API runs only after
     * eight total characters have been entered.
     */
...
const teacherValidationSchema = Yup.object({
  ...
  employee_code: Yup.string()
    ...
    .test(
      "minimum-six-digits",
      "Employee code must contain at least 6 digits",
      (value) => {
  ...
  joining_date: Yup.date()
    ...
    .typeError("Enter a valid joining date")
    .optional(),
...
function teacherToInitialValues(
  ...
  return {
    first_name:
      safeValue(teacher.first_name),

    last_name:
      safeValue(teacher.last_name),

    employee_code:
      safeValue(teacher.employee_code),

    email:
      safeValue(teacher.email),

    phone:
      safeValue(teacher.phone),

    alternate_phone:
      safeValue(teacher.alternate_phone),

    gender:
      safeValue(teacher.gender),

    date_of_birth:
      safeValue(teacher.date_of_birth),

    blood_group:
      safeValue(teacher.blood_group),

    marital_status:
      safeValue(teacher.marital_status),

    current_address:
      safeValue(teacher.current_address),
    ...
    city:
      safeValue(teacher.city),

    state:
      safeValue(teacher.state),

    country:
      safeValue(teacher.country),

    pincode:
      safeValue(teacher.pincode),

    qualification:
      safeValue(teacher.qualification),

    specialization:
      safeValue(teacher.specialization),

    experience_years:
      safeValue(teacher.experience_years),

    joining_date:
      safeValue(teacher.joining_date),
    ...
    status:
      safeValue(teacher.status) ||
      "active",

    basic_salary:
      safeValue(teacher.basic_salary),

    bank_name:
      safeValue(teacher.bank_name),
    ...
    ifsc_code:
      safeValue(teacher.ifsc_code),

    pan_number:
      safeValue(teacher.pan_number),
...
export default function Teachers() {
  ...
  const editInitialValues = useMemo(() => {
    return editItem
      ? teacherToInitialValues(editItem)
      : undefined;
  }, [editItem]);
          ...
          <div className="max-h-[70vh] overflow-y-auto pr-2">
            ...
            <div className="space-y-6 text-sm">
              ...
              <InfoSection title="Address Details">
                ...
                <Info
                  label="State"
                  value={viewItem.state}
                />

                <Info
                  label="Country"
                  value={viewItem.country}
                />

                <Info
                  label="Pincode"
                  value={viewItem.pincode}
                />
              </InfoSection>
