...
export default function Exam() {
    ...
    const exams = useAppSelector((state :any) => state.exam.exams);
    ...
    const visibleExams = useMemo(() => {
        ...
        return exams.filter((exam: any) =>
            [exam.name, exam.exam_type, exam.status]
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (!form.name.trim() || !form.exam_type.trim()) {
            setError("Exam name and exam type are required.");
            return;
        }
    ...
    const getExamFromRow = (row: Record<string, unknown>) => {
        ...
        return exams.find((exam: any) => exam.id === id) ?? null;
    };
